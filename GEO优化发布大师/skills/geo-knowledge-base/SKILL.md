...
批准不得直接覆盖原始抓取卡。把人工决定追加到项目的 `approval_events.jsonl`，事件至少记录事实卡 ID、准确允许措辞、渠道、负责人和复核日期；工具根据原始卡与最新事件计算有效状态。这样可以保留历史并避免把“页面可见”误改成“长期有效”。

事实闸门按声明执行，不按整篇文章类型豁免。建议或观点中如果包含价格、时效、能力、规则、因果关系或产品隐含能力，仍要拆出 `fact` 声明并绑定事实卡。
