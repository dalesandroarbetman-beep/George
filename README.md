...
- `video-script-workflow/`：视频提取、翻译、原创改写、结构分析和多平台发布方案。
- `jewelry-listing/`：单 SKU 饰品标题审核与上品包生成。
- `GEO优化发布大师/`：公开站点扫描、事实卡、论坛内容和人工发布包。
- `tiktok-collector/`：基于 TikTok 公开数据整理候选素材，不下载视频。
...
`GEO优化发布大师/skills/` 包含公开站点扫描、事实卡、内容策划、写作、质量审查和人工发布包生成。发布脚本只生成本地审核材料和待办队列，不登录论坛、不自动发帖，也不绕过平台限制。数据合同位于 `GEO优化发布大师/contracts/`。

## 安全边界

推送前会拒绝媒体源文件、密钥、Cookie、浏览器缓存和私密配置。`.env.local`、后台会话、商品原始图库与本地生成的业务数据只保留在本机。
